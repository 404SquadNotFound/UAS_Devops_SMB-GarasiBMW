...
<div x-data="{ 
    ...
    {{-- ✅ DIUBAH: rekap mulai dari null, akan diisi dari API --}}
    rekap: {
        mingguan: null,
        bulanan: null,
        tahunan: null
    ...
    {{-- ✅ BARU: Fungsi fetch rekap dari database via AJAX --}}
    async fetchRekap(type) {
        if (this.rekap[type] !== null) return; // Cache: skip jika sudah pernah di-fetch
...
">
    ...
    <div class="flex items-center justify-between mb-5">
        ...
        <div class="flex items-center gap-2.5">
            <button class="flex items-center gap-2 px-5 py-[11px] bg-white border border-[#D9E2EC] rounded-[10px] font-bold text-[13px] text-[#213F5C] shadow-sm hover:bg-gray-50 transition-all">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>
                Filter
            </button>
            <button class="flex items-center gap-2 px-5 py-[11px] bg-white border border-[#D9E2EC] rounded-[10px] font-bold text-[13px] text-[#213F5C] shadow-sm hover:bg-gray-50 transition-all">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                Export
            </button>

            {{-- ✅ DIUBAH: Tambah fetchRekap('mingguan') saat tombol diklik --}}
    ...
    <div class="bg-[#4D82F3] rounded-[16px] p-6 mb-6 flex items-center justify-between text-white shadow-sm mt-4">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 bg-white/20 rounded-[14px] flex items-center justify-center backdrop-blur-sm border border-white/10">
        ...
        <div class="flex items-center gap-1.5 bg-white/10 p-1.5 rounded-[12px] backdrop-blur-sm border border-white/10">
            <a href="?weekOffset={{ $weekOffset - 1 }}" class="w-9 h-9 flex items-center justify-center bg-white/20 hover:bg-white/30 rounded-lg transition-colors">
    ...
    <div class="bg-white rounded-xl border border-[#D9E2EC] p-4 mb-5 flex items-center gap-5 shadow-sm overflow-x-auto">
        ...
        <div class="flex items-center gap-3.5 whitespace-nowrap">
            ...
            <div class="bg-[#FFF4D9] text-[#FFB800] px-3.5 py-1.5 rounded-lg text-[12px] font-bold flex items-center gap-2">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 text-[13px]">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-5 font-bold text-[#213F5C]">{{ $emp['name'] }}</td>
                        
                        @foreach(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $day)
                            @php 
                                $attendance = $emp['attendances'][$day]; 
                                $currentDate = $periodDates[array_search($day, array_column($periodDates, 'day_key'))]['date'] ?? '';
                                $dayNameIndo = $periodDates[array_search($day, array_column($periodDates, 'day_key'))]['name'] ?? '';
                                
                                $h = ''; $m = ''; $ampm = 'AM';
                                if($attendance && $attendance['clock_in']) {
                                    $timeObj = \Carbon\Carbon::parse($attendance['clock_in']);
                                    $h = $timeObj->format('g'); 
                                    $m = $timeObj->format('i');
                                    $ampm = $timeObj->format('A');
                                }
                            @endphp
                                ...
                                @if($attendance)
                                    <button 
                                        @click="
                                            ...
                                            form.day_name = '{{ $dayNameIndo }}';
                                            form.status = '{{ $attendance['status'] }}';
                                            form.hour = '{{ $h }}';
                                            form.minute = '{{ $m }}';
                                            form.ampm = '{{ $ampm }}';
                                            form.reason = '{{ $attendance['reason'] ?? '' }}';
                                            form.fileName = '';
                                            openInputModal = true;
                                        "
                                        class="w-full inline-block py-2 px-3 rounded-xl text-[12px] font-bold border transition-all
                                        @if($attendance['status'] == 'Hadir') bg-[#F0FFF4] text-[#22C55E] border-[#22C55E]
                                        @elseif($attendance['status'] == 'Izin Terlambat') bg-[#f0f0ff5d] text-[#393947] border-[#9c9faf]
                                        @elseif($attendance['status'] == 'Libur') bg-[#F3E8FF] text-[#A855F7] border-[#D8B4FE]
                                        @elseif($attendance['status'] == 'Terlambat') bg-[#FFF4D9] text-[#FFB800] border-[#FFB800]
                                        @elseif($attendance['status'] == 'Sakit') bg-[#FFEAEA] text-[#FF4D4D] border-[#FF4D4D]
                                        @elseif($attendance['status'] == 'Cuti') bg-[#EAF2FF] text-[#1273EB] border-[#1273EB]
                                        @endif"
                                    >
                                        @if(in_array($attendance['status'], ['Hadir', 'Terlambat', 'Izin Terlambat']))
                                            <span class="flex items-center justify-center gap-1">
                                                @if($attendance['status'] == 'Terlambat')
                                                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z"></path></svg>
                                                @endif
                                                {{ $attendance['clock_in'] ? \Carbon\Carbon::parse($attendance['clock_in'])->format('H:i') : $attendance['status'] }}
                                            </span>
                                        @else
                                            {{ $attendance['status'] }}
                                        @endif
                                ...
                                @else
                                    <button 
                                        @click="
                                            ...
                                            openInputModal = true;
                                        "
                                        class="w-full py-2 px-3 bg-white text-gray-400 border border-dashed border-gray-300 rounded-xl text-[11px] hover:bg-gray-50 font-medium transition-all"
                                    >
                                        + Tambah
                                    </button>
                                @endif
    ...
    {{-- ======================================================= --}}
    <div x-show="openInputModal" class="fixed inset-0 z-[999] flex items-center justify-center overflow-y-auto" style="display: none;">
        <div class="fixed inset-0 bg-black/40 backdrop-blur-sm" x-transition.opacity @click="openInputModal = false"></div>
        ...
        <div class="relative bg-white w-full max-w-md mx-4 rounded-[24px] shadow-2xl p-8" x-transition>
            ...
            <form action="{{ route('attendance.storeManual') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-5" x-data="{ dropdownOpen: false }">
                    ...
                    <div class="relative w-full">
                        ...
                        <button type="button" @click="dropdownOpen = !dropdownOpen" @click.outside="dropdownOpen = false"
                            ...
                            <svg class="w-5 h-5 transition-transform duration-200" :class="dropdownOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-show="dropdownOpen" x-transition.opacity.duration.200ms
                            ...
                            <button type="button" @click="form.status = 'Izin Terlambat'; dropdownOpen = false" 
                                class="w-full text-left px-4 py-3 rounded-[12px] font-bold text-[15px] border-2 bg-[#f0f0ff5d] text-[#393947] border-[#9c9faf] hover:opacity-70 transition-opacity">Izin Terlambat</button>
                            ...
                            <button type="button" x-show="form.status === 'Terlambat'" @click="dropdownOpen = false" 
                                class="w-full text-left px-4 py-3 rounded-[12px] font-bold text-[15px] border-2 bg-[#FFF4D9] text-[#FFB800] border-[#FFB800]">Terlambat (Dideteksi Sistem)</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6" x-show="['Hadir', 'Terlambat', 'Izin Terlambat'].includes(form.status)" x-transition>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="number" x-model="form.minute" 
                            ...
                            class="w-20 px-4 py-3 border border-[#D9E2EC] rounded-xl outline-none text-[14px] text-center focus:border-[#1273EB]">
                        <select x-model="form.ampm" class="px-4 py-3 border border-[#D9E2EC] rounded-xl outline-none text-[14px] focus:border-[#1273EB] bg-white flex-1">
                            <option value="AM">AM</option>
                            <option value="PM">PM</option>
                        </select>
                    </div>
                    <div x-show="form.status === 'Terlambat'" x-transition class="mt-3 w-full bg-[#FFF4D9] text-[#FFB800] text-[11px] font-bold p-3 rounded-xl flex items-center justify-center gap-2 border border-[#FDE68A]">
                ...
                <div class="mb-6 space-y-4" x-show="['Sakit', 'Cuti', 'Izin Terlambat'].includes(form.status)" x-transition>
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Alasan Keterangan</label>
                        <textarea name="reason" x-model="form.reason" rows="3" placeholder="Masukkan alasan yang relevan..." class="w-full px-4 py-3 border border-[#D9E2EC] rounded-xl outline-none text-[13px] focus:border-[#1273EB] resize-none"></textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Lampiran Bukti (Opsional)</label>
                        <div class="relative w-full border-2 border-dashed border-[#D9E2EC] rounded-xl hover:bg-gray-50 transition-colors p-4 text-center cursor-pointer">
                            <input type="file" name="photo" @change="form.fileName = $event.target.files[0].name" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <span class="text-[12px] font-medium text-gray-500" x-text="form.fileName ? form.fileName : 'Klik atau seret file ke sini'"></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex gap-3 mt-8">
                    <button type="button" @click="openInputModal = false" class="flex-1 py-3.5 bg-[#FFEAEA] text-[#FF4D4D] rounded-xl font-bold text-[14px] hover:bg-[#FFE0E0] transition-all">Batal</button>
                    <button type="submit" class="flex-1 py-3.5 bg-[#1273EB] text-white rounded-xl font-bold text-[14px] hover:bg-[#0E62CC] transition-all shadow-md shadow-blue-200">Simpan Absen</button>
                </div>
            </form>
        </div>
    </div>
    ...
    {{-- ======================================================= --}}
    {{-- 6. MODAL REKAP — ✅ DIUBAH: Data dari database via AJAX --}}
    {{-- ======================================================= --}}
        ...
        <div class="relative bg-white w-full max-w-4xl mx-4 rounded-[32px] shadow-2xl p-10" x-transition>
            ...
            <div class="flex items-start gap-5 mb-8">
                ...
                <div>
                    ...
                    {{-- Total: tampil data atau skeleton loading --}}
            ...
            <div class="w-full bg-[#F8FAFC] border border-gray-100 p-1.5 rounded-2xl mb-8 flex items-center">
                {{-- ✅ DIUBAH: Tiap tab trigger fetchRekap --}}
            ...
            <div x-show="rekapLoading" class="grid grid-cols-2 gap-5 mb-8">
                ...
                <div class="col-span-2 rounded-[24px] p-6 border border-gray-100 animate-pulse bg-gray-50 h-[112px]"></div>
            </div>

            {{-- Card Grid (muncul setelah data tersedia) --}}
            <div x-show="rekap[activeTab] && !rekapLoading" class="grid grid-cols-2 gap-5 mb-8">

                {{-- Card: Hadir --}}
                <div class="bg-[#F0FFF4] border border-[#BBF7D0] rounded-[24px] p-6 flex items-center justify-between shadow-sm">
                    <div class="flex items-center gap-4">
                ...
                {{-- Card: Terlambat --}}
                <div class="bg-[#FFF4D9] border border-[#FDE68A] rounded-[24px] p-6 flex items-center justify-between shadow-sm">
                    <div class="flex items-center gap-4">
                        <div class="w-14 h-14 bg-[#FDE68A] rounded-2xl flex items-center justify-center text-[#FFB800] border border-yellow-200">
                            <svg class="w-7 h-7" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z"></path></svg>
                        </div>
                        <div>
                ...
                {{-- Card: Sakit --}}
                <div class="bg-[#FFEAEA] border border-[#FCA5A5] rounded-[24px] p-6 flex items-center justify-between shadow-sm">
                    <div class="flex items-center gap-4">
                        <div class="w-14 h-14 bg-[#FCA5A5] rounded-2xl flex items-center justify-center text-[#FF4D4D] border border-red-200">
                ...
                {{-- Card: Cuti --}}
                <div class="bg-[#EAF2FF] border border-[#BFDBFE] rounded-[24px] p-6 flex items-center justify-between shadow-sm">
                    <div class="flex items-center gap-4">
                ...
                {{-- Card: Libur (full width) --}}
                <div class="col-span-2 bg-[#F3E8FF] border border-[#D8B4FE] rounded-[24px] p-6 flex items-center justify-between shadow-sm">
                    <div class="flex items-center gap-4">
                        <div class="w-14 h-14 bg-[#D8B4FE] rounded-2xl flex items-center justify-center text-[#A855F7] border border-purple-200">
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-100">
                ...
                <button @click="openModal = false" class="px-12 py-3.5 bg-[#1273EB] text-white rounded-2xl font-bold text-[15px] shadow-lg shadow-blue-200 hover:bg-[#0E62CC] transition-all">
                    Tutup
                </button>
            </div>
        </div>
