                    ...
                    <div class="relative" id="customerDropdownWrapper">
                        <div class="relative">
                            <input type="text" id="customerSearch" placeholder="Ketik nama atau nomor telepon pelanggan..."
                                autocomplete="off" value="{{ $antrian->customer->name ?? '' }} - {{ $antrian->customer->phone ?? '' }}"
                            ...
                            <span id="customerSearchClear" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 cursor-pointer hover:text-red-400 text-lg leading-none" onclick="clearCustomer()">×</span>
                        </div>
                    ...
                    <div class="relative" id="vehicleDropdownWrapper">
                        <div class="relative">
                            <input type="text" id="vehicleSearch" placeholder="Cari model atau nopol kendaraan..."
                                autocomplete="off" value="{{ $antrian->vehicle->model ?? '' }} - {{ $antrian->vehicle->license_plate ?? '' }}"
                                class="w-full px-5 py-3.5 bg-[#F9FBFF] border border-[#E5E9F2] rounded-xl outline-none focus:border-[#1273EB] focus:ring-2 focus:ring-[#1273EB]/10 transition-all text-[14px] text-[#213F5C] placeholder-gray-300 pr-10">
                            <span id="vehicleSearchClear" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 cursor-pointer hover:text-red-400 text-lg leading-none" onclick="clearVehicle()">×</span>
                        </div>
...
@section('content')
    ...
    <script>
        ...
        // Event listener inputNamaBarang dihapus karena fungsi pencarian digabung di stokSearch

        // ── Dirty flag & Prevent default enter ────────────────────────────────────
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                if (['input', 'textarea', 'select'].includes(tag)) { e.preventDefault(); return; }
                e.preventDefault();
            }
        });
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                ...
                if (vehicleData && selectedCustomer) {
                    const mappedVehicle = selectedCustomer.vehicles?.find(v => v.id === vehicleData.vehicles_id);
                    if (mappedVehicle) {
                        ...
                        selectVehicle({
                            ...
                            label: vehicleData.model + ' — ' + vehicleData.license_plate
                        });
                    }
                }
