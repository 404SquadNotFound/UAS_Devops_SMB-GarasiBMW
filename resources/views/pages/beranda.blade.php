...
@section('content')
    ...
    <script>
        ...
        /* ── 1. Load Nama Pegawai dari localStorage ── */
        ...
        /* ── 2. Render Tabel Antrian dari localStorage ── */
        function renderBerandaTable(filterText = '') {
            ...
            /* Update stat cards & banner dari data localStorage */
            updateStatCards(filtered, list.length);
            ...
            filtered.forEach(item => {
                const badgeCls = statusMap[item.status] ?? 'bg-gray-100 text-gray-500 border-gray-200';
            ...
            if (fromEl)  fromEl.innerText  = filtered.length > 0 ? 1 : 0;
            if (toEl)    toEl.innerText    = filtered.length;
            if (totalEl) totalEl.innerText = filtered.length;
        }

        /* ── 3. Update Stat Cards & Banner ── */
        function updateStatCards(items, totalSemua) {
            const bannerEl = document.getElementById('bannerTotalKendaraan');
            if (bannerEl) bannerEl.innerText = totalSemua;

            const count = (label) =>
                items.filter(i => (i.status ?? '') === label).length;
