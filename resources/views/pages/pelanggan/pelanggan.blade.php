...
@section('content')
    ...
    async function fetchCustomers(search = '', car_type_id = '', series = '', page = 1) {
        ...
        // Lacak URL yang benar-benar ditembak oleh JavaScript
        const targetUrl = `/api/customers?limit=10&search=${search}&car_type_id=${car_type_id}&series=${series}&page=${page}`;
        console.log("=== MENEMBAK API ===");
            ...
            if (res.ok) {
                ...
                items.forEach(c => {
                    ...
                    const models = c.vehicles.length > 0
                        ? c.vehicles.map(v => `<div class="mb-1 last:mb-0 font-bold">${v.model || (v.carType ? v.carType.name : '-')}</div>`).join('')
                        : '<span class="text-gray-300">-</span>';
    ...
    function applyFilter() {
        ...
        // Lacak nilai value yang didapat dari elemen HTML select
        console.log("=== TOMBOL TERAPKAN DIKLIK ===");
        console.log("Data Filter Didapat:", { search, car_type_id, series });
    ...
    async function loadOptions() {
        try {
            ...
            if (resCarTypes.ok) {
                ...
                select.innerHTML = '<option value="">Semua Model</option>';
            ...
            if (resSeries.ok) {
                ...
                select.innerHTML = '<option value="">Semua Seri</option>';
                (resultSeries.data || []).forEach(s => {
                    const opt = document.createElement('option');
                    // Amankan jika s berupa object atau string langsung
                    const val = (typeof s === 'object' && s !== null) ? (s.series || s.name) : s;
                    opt.value = val;
                    opt.text = val;
                    select.appendChild(opt);
                });
